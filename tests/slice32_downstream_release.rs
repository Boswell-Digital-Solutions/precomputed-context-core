...
fn valid_downstream_release_builds_and_publishes() -> Result<(), Box<dyn Error>> {
    ...
    assert!(output_dir
        .join("release_package/consumer_acknowledgment_receipt.json")
        .exists());
    assert!(output_dir
        .join("release_package/return_channel_closure_receipt.json")
        .exists());
    Ok(())
}
...
fn closure_drift_fails_closed() -> Result<(), Box<dyn Error>> {
    ...
    assert!(
        error
            .to_string()
            .contains("closure receipt does not match reconstructed continuity"),
